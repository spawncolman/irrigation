...
void Image_Init(uint8_t *image, uint16_t width, uint16_t height,
    ...
    // Image.byte_h); printf(" EPD_width / 8 = %d\r\n",  122 / 8);
...
parameter:
    image   :   Pointer to the image cache
******************************************************************************/
void Gui_SelectImage(uint8_t *image) { Image.img = image; }
...
parameter:
    ...
    y2   :   y end point
******************************************************************************/
void Gui_ClearWindows(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2,
                      uint16_t color) {
    uint16_t x, y;
    for (y = y1; y < y2; y++) {
        for (x = x1; x < x2; x++) { // 8 pixel =  1 byte
...
******************************************************************************/
void Gui_Draw_Point(uint16_t x, uint16_t y, uint16_t color, DOT_PIXEL Dot_Pixel,
                    DOT_STYLE Dot_Style) {
    int16_t xd, yd;
    if (x > Image.w || y > Image.h) {
...
******************************************************************************/
void Gui_Draw_Circle(uint16_t x, uint16_t y, uint16_t radius, uint16_t color,
                     DRAW_FILL fill, DOT_PIXEL Dot_Pixel) {
...
void Gui_Draw_Str(uint16_t x, uint16_t y, const char *str_char, FONT *Font,
    ...
    while (*str_char != '\0') {
        if ((dx + Font->w) >
            Image
                .w) // wrap the line if the x direction has reached the boundary
        {
            dx = x;
            dy += Font->h;
        }
        if ((dy + Font->h) >
            Image
                .h) // wrap the line if the y direction has reached the boundary
        {
            dy = y;
            dx += Font->w;
        }
        Gui_Draw_Char(dx, dy, *str_char++, Font, Color_Background,
                      Color_Foreground);
        dx += Font->w;
    }
}
...
void Gui_Draw_Num(uint16_t x, uint16_t y, int32_t nummber, FONT *Font,
                  uint16_t Color_Background, uint16_t Color_Foreground) {
    ...
    // show
    Gui_Draw_Str(x, y, (const char *)pStr, Font, Color_Background,
                 Color_Foreground);
}
...
void Gui_draw_time(uint16_t x1, uint16_t y1, DRAW_TIME *pTime, FONT *Font,
    ...
    Gui_Draw_Char(x1, y1, value[pTime->Hour / 10], Font, Color_Background,
                  Color_Foreground);
    Gui_Draw_Char(x1 + dx, y1, value[pTime->Hour % 10], Font, Color_Background,
                  Color_Foreground);
    Gui_Draw_Char(x1 + dx + dx / 4 + dx / 2, y1, ':', Font, Color_Background,
                  Color_Foreground);
    Gui_Draw_Char(x1 + dx * 2 + dx / 2, y1, value[pTime->Min / 10], Font,
                  Color_Background, Color_Foreground);
    Gui_Draw_Char(x1 + dx * 3 + dx / 2, y1, value[pTime->Min % 10], Font,
                  Color_Background, Color_Foreground);
    Gui_Draw_Char(x1 + dx * 4 + dx / 2 - dx / 4, y1, ':', Font,
                  Color_Background, Color_Foreground);
    Gui_Draw_Char(x1 + dx * 5, y1, value[pTime->Sec / 10], Font,
                  Color_Background, Color_Foreground);
    Gui_Draw_Char(x1 + dx * 6, y1, value[pTime->Sec % 10], Font,
                  Color_Background, Color_Foreground);
}

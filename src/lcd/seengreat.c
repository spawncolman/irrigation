...
#define EPD_W21_MOSI_0 digitalWrite(CUSTOMLCD_DIN_PIN, LOW)
#define EPD_W21_MOSI_1 digitalWrite(CUSTOMLCD_DIN_PIN, HIGH)

#define EPD_W21_CLK_0 digitalWrite(CUSTOMLCD_CLK_PIN, LOW)
#define EPD_W21_CLK_1 digitalWrite(CUSTOMLCD_CLK_PIN, HIGH)

#define EPD_W21_CS_0 digitalWrite(CUSTOMLCD_CS_PIN, LOW)
#define EPD_W21_CS_1 digitalWrite(CUSTOMLCD_CS_PIN, HIGH)

#define EPD_W21_DC_0 digitalWrite(CUSTOMLCD_DC_PIN, LOW)
#define EPD_W21_DC_1 digitalWrite(CUSTOMLCD_DC_PIN, HIGH)
#define EPD_W21_RST_0 digitalWrite(CUSTOMLCD_RST_PIN, LOW)
#define EPD_W21_RST_1 digitalWrite(CUSTOMLCD_RST_PIN, HIGH)
#define isEPD_W21_BUSY digitalRead(CUSTOMLCD_BUSY_PIN)
...
previous picture. 2.When the partial refresh is performed, the screen does not
flash. 3.After the e-paper is refreshed, you need to put it into sleep mode,
please do not delete the sleep command. 4.Please do not take out the electronic
paper when power is on. 5.Wake up from sleep, need to re-initialize the e-paper.
...
void seengreat_setup(void) {
    pinMode(CUSTOMLCD_BUSY_PIN, INPUT);
    pinMode(CUSTOMLCD_RST_PIN, OUTPUT);
    pinMode(CUSTOMLCD_DC_PIN, OUTPUT);
    pinMode(CUSTOMLCD_CS_PIN, OUTPUT);
    pinMode(CUSTOMLCD_CLK_PIN, OUTPUT);
    pinMode(CUSTOMLCD_DIN_PIN, OUTPUT);
    pinMode(CUSTOMLCD_TEST_PIN, OUTPUT);
}

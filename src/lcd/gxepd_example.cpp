#if 1
// PartialUpdateExample : example for Waveshare 1.54", 2.31" and 2.9" e-Paper
// and the same e-papers from Dalian Good Display Inc.
//
// Created by Jean-Marc Zingg based on demo code from Good Display for
// GDEP015OC1.
//
...
//#include <GxDEPG0213BN/GxDEPG0213BN.h>    // 2.13" b/w 128x250, SSD1680, TTGO
// T5 V2.4.1, V2.3.1 #include <GxGDEH029A1/GxGDEH029A1.h>      // 2.9" b/w
...
//#include <GxDEPG0290BS/GxDEPG0290BS.h>    // 2.9" b/w Waveshare variant, TTGO
// T5 V2.4.1 2.9" #include <GxGDEW026T0/GxGDEW026T0.h>      // 2.6" b/w #include
//<GxDEPG0266BN/GxDEPG0266BN.h>      // 2.66" b/w 152x296, SSD1680, TTGO T5
// V2.66, TTGO T5 V2.4.1 #include <GxGDEW027W3/GxGDEW027W3.h>      // 2.7" b/w
...
GxIO_Class io(SPI, /*CS=D8*/ SS, /*DC=D3*/ 0,
              /*RST=D4*/ 2); // arbitrary selection of D3(=0), D4(=2), selected
                             // for default of GxEPD_Class
GxEPD_Class display(io, /*RST=D4*/ 2,
...
GxIO_Class io(SPI, /*CS=5*/ SS, /*DC=*/CUSTOMLCD_DC_PIN,
              /*RST=*/CUSTOMLCD_RST_PIN); // arbitrary selection of 17, 16
GxEPD_Class display(io, /*RST=*/CUSTOMLCD_RST_PIN,
                    /*BUSY=*/CUSTOMLCD_BUSY_PIN); // arbitrary selection of (16), 4

// for LILYGO® TTGO T5 2.66 board uncomment next two lines instead of previous
// two lines
// GxIO_Class io(SPI, /*CS=5*/ SS, /*DC=*/ 19, /*RST=*/ 4); // LILYGO® TTGO
// T5 2.66 GxEPD_Class display(io, /*RST=*/ 4, /*BUSY=*/ 34); // LILYGO® TTGO
// T5 2.66
...
GxIO_Class io(SPI, /*CS=*/SS, /*DC=*/PE15,
              /*RST=*/PE14); // DC, RST as wired by DESPI-M01
GxEPD_Class display(io, /*RST=*/PE14,
                    /*BUSY=*/PE13); // RST, BUSY as wired by DESPI-M01
...
GxIO_Class io(SPI, /*CS=*/SS, /*DC=*/8,
              /*RST=*/9); // arbitrary selection of 8, 9 selected for default of
                          // GxEPD_Class
// GxIO_Class io(SPI, /*CS=*/ 10, /*DC=*/ 8, /*RST=*/ 9); // arbitrary selection
// of 8, 9, CS on 10 (for CS same as on UNO, for SPI on ICSP use)
...
static void showPartialUpdate(void);

void gxepd_example_setup(void) {
    Serial.begin(115200);
    Serial.println();
...
void gxepd_example_loop(void) {
#if defined(__AVR) || false
    showPartialUpdatePaged();
#elif (defined(_GxGDEW075Z09_H_) || defined(_GxGDEW075Z08_H_)) &&              \
    (defined(ESP8266) || defined(ARDUINO_ARCH_STM32F1))
    showPartialUpdatePaged();
...
void showPartialUpdate(void) {
    ...
    for (uint16_t r = 0; r < 4; r++) {
        ...
        display.setRotation(0);
        display.drawExampleBitmap(BitmapExample1, 0, 0, GxEPD_WIDTH,
                                  GxEPD_HEIGHT, GxEPD_BLACK);
        display.updateWindow(0, 0, GxEPD_WIDTH, GxEPD_HEIGHT, false);
        display.setRotation(r);
    ...
    for (uint16_t r = 0; r < 4; r++) {
        ...
        display.updateWindow(0, 0, GxEPD_WIDTH, GxEPD_HEIGHT, false);
        display.setRotation(r);
        if (box_x >= display.width())
            continue; // avoid delay
        for (uint16_t i = 1; i <= 10; i++) {
            display.fillRect(box_x, box_y, box_w, box_h, GxEPD_WHITE);
            display.setCursor(box_x, cursor_y);
            display.print(value * i, 2);
...
void showPartialUpdate_75Z09() {
    ...
    display.setTextColor(GxEPD_BLACK);
    display.setRotation(0);
    // draw background, partial update to full screen to preset for partial
    // update of box window (updateWindow() would clear display if partial
    // update not set, to avoid strange background effect)
    ...
    for (uint16_t r = 0; r < 4; r++) {
        // reset the background
        display.setRotation(0);
        display.drawExamplePicture_3C(BitmapPicture_3C,
                                      sizeof(BitmapPicture_3C),
                                      GxEPD::bm_partial_update);
        display.setRotation(r);
        for (uint16_t i = 1; i <= 10; i++) {
            display.fillRect(box_x, box_y, box_w, box_h, GxEPD_WHITE);
...
#if defined(__AVR) ||                                                          \
    (defined(_GxGDEW075Z09_H_) || defined(_GxGDEW075Z08_H_)) &&                \
        (defined(ESP8266) || defined(ARDUINO_ARCH_STM32F1)) ||                 \
    false
...
void showPartialUpdatePaged() {
    ...
    for (uint16_t r = 0; r < 4; r++) {
        display.setRotation(r);
        display.drawPagedToWindow(showBlackBoxCallback, box_x, box_y, box_w,
                                  box_h, GxEPD_BLACK);
        delay(1000);
        display.drawPagedToWindow(showBlackBoxCallback, box_x, box_y, box_w,
                                  box_h, GxEPD_WHITE);
    ...
    for (uint16_t r = 0; r < 4; r++) {
        ...
        for (uint16_t i = 1; i <= 10; i++) {
            uint32_t v = value * i;
            display.drawPagedToWindow(showValueBoxCallback, box_x, box_y, box_w,
                                      box_h, v);
            delay(500);
        }
        delay(1000);
